...
public class GrassPainter : MonoBehaviour
{
    ...
    public float raycastHitLength = 200f;
    ...
    void OnScene(SceneView scene)
    {
        ...
        if ((Selection.Contains(gameObject)))
        {
            ...
            if (Physics.Raycast(rayGizmo, out hitGizmo, raycastHitLength, hitMask.value))
            {
            ...
            if (e.type == EventType.MouseDrag && e.button == 1 && toolbarInt == 0)
            {
                ...
                for (int k = 0; k < density; k++)
                {
                    ...
                    if (Physics.Raycast(ray, out terrainHit, raycastHitLength, hitMask.value) && i < grassLimit && terrainHit.normal.y <= (1 + normalLimit) && terrainHit.normal.y >= (1 - normalLimit))
                    {
            ...
            if (e.type == EventType.MouseDrag && e.button == 1 && toolbarInt == 1)
            {
                ...
                if (Physics.Raycast(ray, out terrainHit, raycastHitLength, hitMask.value))
                {
                    hitPos = terrainHit.point;
                    hitPosGizmo = hitPos;
                    hitNormal = terrainHit.normal;
