...
public class GrassComputeScript : MonoBehaviour
{
    ...
    [SerializeField]
    ShaderInteractor[] interactorArray;
    Vector4[] interactorPositions = new Vector4[100];
    ...
    private void SetGrassDataUpdate()
    {
        ...
        if (interactorArray.Length > 0)
        {
            for(int i=0; i<interactorArray.Length; i++){
                interactorPositions[i] = interactorArray[i].transform.position;
            }

            m_InstantiatedComputeShader.SetVectorArray("_PositionsMoving", interactorPositions);
            m_InstantiatedComputeShader.SetFloat("_InteractorAmount", interactorArray.Length);
        }
        else
        {
